...
const CATS = [
  { name: 'Voice assistants / smart speakers & displays', description: 'Alexa, Google Nest, HomePod' },
  { name: 'Smart lighting', description: 'bulbs, switches, strips (Philips Hue, LIFX)' },
  { name: 'Security', description: 'cameras, video doorbells, alarm systems (Ring, Arlo, Nest)' },
  { name: 'Smart locks & access', description: 'deadbolts, keypads, garage openers' },
  { name: 'Climate control', description: 'smart thermostats, AC controllers, fans (Nest, Ecobee)' },
  { name: 'Sensors', description: 'motion, door/window, temperature, water leak, air quality' },
  { name: 'Smart plugs & energy', description: 'outlets, power strips, energy monitors' },
  { name: 'Hubs & controllers', description: 'SmartThings, Home Assistant, Hubitat, Matter/Thread hubs' },
  { name: 'Robot vacuums & appliances', description: 'vacuums, mops, smart fridges, ovens' },
  { name: 'Entertainment', description: 'streaming devices, smart TVs, multi-room audio' },
  { name: 'Wearables & health', description: 'sleep, presence detection' },
  { name: 'Connectivity standards', description: 'Wi-Fi, Zigbee, Z-Wave, Matter, Thread' },
];
...
for (const { name, description } of CATS) {
  ...
  if (found.j?.data?.length) {
    ...
    if (!existingDesc) body.data.description = description; // don't clobber an existing description
    const up = await api(`/api/gatsby-categories/${cat.documentId}`, { method: 'PUT', body: JSON.stringify(body) });
    console.log(`  = "${name}" existed → linked to ${SITE_KEY} (${up.status})${existingDesc ? ' [kept its description]' : ''}`);
    continue;
  }
